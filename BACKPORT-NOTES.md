...
| Side-channel `<track>` ownership of all text subtitles (no manifest-managed text tracks) | ✅ Done | The architectural conflict was real in 2.0, not just hypothetical -- 2.0 inherited `SubtitleMethod=Hls` + `ManifestSubtitles=vtt` from the 1.6.3 port. Backend manifest text-sub params removed in commit `345326b`. **Frontend architecture clarification (added after testing):** programmatically setting `textTrack.mode = 'showing'` while HLS.js is running causes phantom `seeked` events to fire on the video element, which get re-emitted as user seeks and flood the chat. This was the original reason for keeping text subs out of the dropdown in pre-NewBlade 2.0 -- the CC button is the only safe activation surface because the browser's native gesture path apparently does not trigger the same phantom seeks. NewBlade's `bcafaa6` reintroduced the bug by wiring text-sub selections through the dropdown. Reverted: text subs are once again CC-button-only via the auto-load preload path; dropdown shows PGS / image subs only. |
