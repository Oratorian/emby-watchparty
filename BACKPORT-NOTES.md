...
| Item | Status | Notes |
|---|---|---|
| CC menu label includes Title field plus `[Forced]` and `[External]` markers | ⚠️ Reimagine | In 1.x: `static/js/video.js` `loadAllTextSubtitles()` builds a label string when creating `<track>` elements. **In 2.0 (Vue):** subtitle label construction is likely in a Vue component that creates `<track>` declaratively from a reactive subtitle list. Same conceptual fix — combine `displayLanguage`, `title`, `isForced`, `isExternal` into the label — but the implementation lives in `frontend/src/...`. |
| Side-channel `<track>` ownership of all text subtitles (no manifest-managed text tracks) | ⚠️ Reimagine | This is the architectural piece that requires 2.0-native judgment. **Read carefully:** the 1.6.6 fix removed manifest text subs because two parallel subtitle delivery systems fought over `textTrack.mode` state. **Verify in 2.0 whether this fight exists at all.** Vue's reactive `<track>` element rendering may behave differently from imperative DOM manipulation. If 2.0's Vue component creates `<track>` elements declaratively from a reactive list, and the manifest path was already removed (per the backend deletion above), the bug class doesn't exist in 2.0. **Don't port the JS fix verbatim.** Port the conceptual model: one and only one subtitle delivery system per video. |
