...
| Side-channel `<track>` ownership of all text subtitles (no manifest-managed text tracks) | ✅ Done | Backend manifest text-sub params removed in commit `345326b`. Frontend `onChangeTextSubtitle` rewritten in commit `2bd8245` to find the preloaded `<track>` by URL and toggle `textTrack.mode` instead of wiping the set. **The dropdown unifies PGS and text subs** -- text picks toggle preloaded `<track>` modes (instant, no stream restart), PGS picks fire `change_streams` for backend re-encode (per-user transcode restart, party unaffected). The dropdown and CC button drive the same underlying preloaded-track set. **History note:** an earlier theory blamed phantom seeks on programmatic `textTrack.mode = 'showing'` and briefly reverted to a CC-only architecture, but the actual root cause was Vite HMR stacking duplicate socket.io listeners across reloads (a single server emit was firing chat side-effects 6x). Fixed in commit `56c341c` by clearing existing listeners at the start of each setup function. The unified dropdown architecture is correct in 2.0 once listener stacking is resolved. |
